...
#include <math.h>
#include<conio.h>
typedef union uwb { unsigned w; unsigned char b[4]; } MD5union;
...
unsigned *calctable( unsigned *k) {
    double s, pwr; int i; pwr = pow( 2, 32);
    for (i=0; i<64; i++) { s = fabs(sin(1+i)); k[i] = (unsigned)( s * pwr ); }
    return k;
}
...
unsigned *md5( const char *msg, int mlen) {
    ...
    static unsigned kspace[64]; static unsigned *k; static DigestArray h;
    DigestArray abcd; DgstFctn fctn; short m, o, g; unsigned f; short *rotn;
    union { MD5union mm; char b[64]; } mm;
    unsigned w[16]; char b[64]; int os = 0, grp, grps, q, p;
    unsigned char *msg2;
    ...
    for (grp=0; grp<grps; grp++) {
        ...
        for (p = 0; p<4; p++) {
            ...
            for (q=0; q<16; q++) {
                g = (m*q + o) % 16;
                f = abcd[1] + rol( abcd[0]+ fctn(abcd)+k[q+16*p] + mm.mm.w[g], rotn[q%4]);
                abcd[0] = abcd[3]; abcd[3] = abcd[2]; abcd[2] = abcd[1]; abcd[1] = f;
            }
        }
